...
<template>
  ...
  <div class="bg-surface-80 dark:bg-surface-950 flex items-center justify-center min-h-screen min-w-[100vw] overflow-hidden">
      ...
      <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
        <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
          <div class="flex flex-col items-center justify-center mb-8">
            ...
            <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">{{ $t('common.app.welcome') }} <strong>Sakola!</strong></div>
          ...
          <div>
            ...
            <InputText id="email1" @keyup.enter="validate" type="text" placeholder="Username" class="w-full md:w-[30rem] mb-8" v-model="store.username" />

            <label for="password1" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Password</label>
            <Password id="password1" @keyup.enter="validate" v-model="store.password" placeholder="Password" :toggleMask="true" class="mb-4" fluid :feedback="false"></Password>
            ...
            <div class="flex items-center justify-between mt-2 mb-8 gap-8">
              ...
              <span class="font-medium no-underline ml-2 text-right cursor-pointer text-primary">{{ $t('auth.forgotPassword') }}</span>
            </div>
            <Button :label="store.message" class="w-full" :disabled="store.disableButton" @click="validate"></Button>
          </div>
        </div>
      </div>
    </div>
